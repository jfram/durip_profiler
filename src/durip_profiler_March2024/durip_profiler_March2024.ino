...
const long SLOW_DEPTH = 1.0;                                             // slowing down for final 2X meters on move down
...
void moveUp (File &data_file) {
    ...
    long move_time = abs(1000*sur_pos*10/up_vel);                   // expected time to complete move to surface 
...
void moveDown (File &data_file) {
    ...
    commandWinch(cmd, data_file);
    if (cur_pos < sur_pos) {
      long i = (100*cur_pos)/sur_pos; // I think I'm exceeding the variable size
    ...
    sprintf_P(cmd, PSTR("t 1\n"));                                      // initiate move
    commandWinch(cmd, data_file);                                       //
    long move_time = abs(1000*inter_bot_pos/(down_vel/(10)));           // expected time to complete move down 
    ...
    sprintf_P(cmd, PSTR("t 1\n"));                                      // initiate move
    commandWinch(cmd, data_file);                                       //
    move_time = abs(1000*(bot_pos - inter_bot_pos)/(bot_vel/(10)));     // expected time to complete move down 
    delay(move_time+3000);                                              //
    eventStatusRegister(data_file);                                     // 
...
void moveDownInitial (File &data_file) {
    ...
    sprintf_P(cmd, PSTR("t 1\n"));                                      // initiate move
    commandWinch(cmd, data_file);                                       //
    long move_time = abs(1000*bot_pos/(bot_vel/(10)));                  // expected time to complete move down
    long i = 0;
    long crnt = 0;
    char *strtokIdx;      
    while (i < move_time) {
...
void moveDownAfta (File &data_file) {                                  // after each profile, keep going lower
    ...
    bot_pos = -sur_pos;
    sprintf_P(cmd, PSTR("s r0xca %ld\n"), sub_pos);                     // spool in a SLOW_DEPTH, bot_pos
    commandWinch(cmd, data_file);                                       //
    ...
    long move_time = abs(1000*SLOW_DEPTH*100L*CPCM/(bot_vel/(10)));     // expected time to complete move down
